...
5. Create free labels such as `screw`, `metal`, `front`, or `defect`.
6. Select several images that show the same target.
7. Apply or remove labels from the label palette.
8. Download the multi-label CSV if needed.
...
- Click a label chip to add or remove that label on the current image or checked images.
- Use `Bulk Label` to apply or remove one label on all checked images.
- Use `Download multi-label CSV` to export labeled rows.

CSV output:

```text
image_id,filename,labels,screw,nut,metal,front,side,target,other,unknown
37,xiao_esp32s3_sense_001_000011.jpg,screw;metal;front,1,0,1,1,0,0,0,0
```

This is multi-label classification, not object detection. If the model must learn object positions, the Phase 2 workflow needs Bounding Box annotations.
